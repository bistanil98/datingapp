...
$destinationPath = public_path('/uploads/profile_ads/');
	...
	/* $tearms = DB::table('top_subida_profile_lists')->get();
	foreach($tearms as $data){
	  $start = $data->from_date;
	  $end = $data->to_date;
		$dateRange = dateRange($start, $end);
		$start1 = strtotime($start);
		$end2 = strtotime($end);
		$seen_days = ceil(abs($start1 - $end2) / 86400);
		...
		foreach($dateRange as $dates){
				...
				->insert([							
					'type' =>$data->type,	
					'profile_ads_id' =>$data->profile_ads_id,
					'from_date' =>$dates,
                      'price'=>round($data->price/$seen_days, 2)
				]);
				}
	}
	dd($tearms);die;
	echo "sdsadsad";die; */
	 ...
	 $tearms = DB::table('profile_ads')->orderBy('id', 'asc')->get();
	 
	//$i = 1; foreach($tearms as $data){
			/* DB::table('schedule')
			->insert([							
			'profile_ads_id' =>$data->id,	
			'days' =>'Lunes',	
			'from_1' =>01+$i,
			'from_2' =>00,
			'unit_1' =>01+$i,
			...
			->insert([							
			'profile_id' =>$data->id,	
			'tariffs_description' =>'1 hour',	
			'tariffs_euro_price' =>$data->tariffs,
			]);
			$i++;
	}
	...
	public function subida_zone(Request $request){     
		...
		public function subida_zone_payment(Request $request){     
			...
			$end =  date('Y-m-d', strtotime('+'.($seen_days-1).' days'));
			$dateRange = dateRange($start, $end);
				foreach($dateRange as $dates){
					DB::table('piesepuerto')
				  ->insert([							
	...
	public function zona_top_payment(Request $request){  
		...
		//	$to_date =  gmdate('Y-m-d',strtotime('+1 month',strtotime($from_date)));
			...
			$dateRange = dateRange($start, $end); */
			$to_date =  date('Y-m-d', strtotime('+'.($seen_days).' days'));
			$dateRange = dateRange($from_date, $to_date);
			foreach($request->input('number_of_photo') as $data){
			
				// new login hour base 
				$to = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', date('Y-m-d H:i:s'));		
				$to_date_2 =  date('Y-m-d H:i:s', strtotime('+'.($seen_days).' days'));				
				$from = \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $to_date_2);
	...
	public function anuncios(Request $request){     
		...
		->select('from_date','to_date','seen_days','ad_active_inactive_status','price','created_at')
		->first();
		if($type=='subida'){
	...
	public function consumo(Request $request) {        
		...
		return view('front.members.consumo',['profile_ads'=>$profile_ads],$data); 
	}
}
