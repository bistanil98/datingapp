...
class AjaxController extends Controller {  
	...
	public function login(Request $request) {  		
		...
		if($get_user->is_email!=1){
			...
			echo 'La dirección de correo electrónico no está verificada.';
		}else if($get_user->status!=1){
			//echo 'Your account is temporarily locked contact your system administrator';			
			echo 'Su cuenta está temporalmente bloqueada, póngase en contacto con el administrador del sistema';
		}else{
			...
			echo 'Detalle de inicio de sesión no válido';
		}
	...
	public function population(Request $request)  {  	
		...
		->get();	
		if(!empty($population)){
		$str .= "<option value=''>Seleccione Población</option>";
	...
	public function populationautocomplete(Request $request)  {  	
		$search_arr=array(); 
         $population2 = array('0'=>$request->population);         
		 $population = DB::table('population');
		 ...
		 $population->where(function($population) use($population2) {
                        foreach($population2 as $term) {
                            $population->orWhere('population.name', 'like', "".$term."%");
                            //$population->orWhere('province.name', 'like', "".$term."%");
                        };
                    });	
		//->where('population.name', 'like', "".$population2."%")
		 $population->orderby('population.name', 'asc');
		 $population->limit(10);
		$data =  $population->get();
		//dd($data);die;
...
//			$search_arr[] = array("value"=>$student->name,"label"=>$student->name.', '.$student->provinceName);
			//}
			if($student->name==$student->provinceName){
			$search_arr[] = array("value"=>$student->name,"label"=>$student->name);
			}else{
			$search_arr[] = array("value"=>$student->name,"label"=>$student->name.', '.$student->provinceName);
			}
	...
	public function populationautocompleteCount(Request $request)  {  
         $population = $request->population;         
		 $search_arr = DB::table('population')
	...
	public function favoritos(Request $request){	
		...
		->first();
			...
			public function service_over2(Request $request){	
			$SelectCategory = strtolower($request->SelectCategory);		 
			...
			dd($over_you);die;
			$over_youArr = array();
			 foreach($over_you as $data){
				if (strpos($SelectCategory,strtolower($data->name)) !== false){
				echo "sdssdsd";die;
					$over_youArr[] = $data->name;
				 }
			 }
				dd($over_youArr);die;
			...
			->where('status','1')
			->get();
			return view('front.ajax.service_over',['over_you'=>$over_you,'services'=>$services]);        
			}
			
				public function service_over(Request $request){	
			$SelectCategory = strtolower($request->SelectCategory);		 
			
			$over_you2=  DB::table('over_you');
			$over_you2->orderby('name','asc');
			$over_you2->where('status','1');
			$over_you2->whereRaw("find_in_set('$SelectCategory',category)");
			/* if($SelectCategory=='escorts'){
				$over_you2->whereRaw("find_in_set('escorts',category)");
			}else if($SelectCategory=='chaperos'){
				$over_you2->whereRaw("find_in_set('chaperos',category)");
			}else if($SelectCategory=='travestis'){
				$over_you2->whereRaw("find_in_set('travestis',category)");
			} */
			$over_you = $over_you2->get();

			$services2=  DB::table('services');
			$services2->orderby('name','asc');
			$services2->where('status','1')		;	
			$services2->whereRaw("find_in_set('$SelectCategory',category)");
			/* if($SelectCategory=='escorts'){
				$services2->whereRaw("find_in_set('escorts',category)");
			}else if($SelectCategory=='chaperos'){
				$services2->whereRaw("find_in_set('chaperos',category)");
			}else if($SelectCategory=='travestis'){
				$services2->whereRaw("find_in_set('travestis',category)");
			} */
			$services = $services2->get();
			return view('front.ajax.service_over',['over_you'=>$over_you,'services'=>$services]);        
			}
		...
		public function pushUrl($id){
			...
			->select('profile_ads.id','profile_ads.title','.profile_ads.province','profile_ads.population','profile_ads.category','province.name')
			->where('profile_ads.id', $id)			
			->first();
			$category = createSlug($profile->category);
			$province = createSlug($profile->name);
			$population = createSlug($profile->population);
			$title = createSlug($profile->title);
			$url = url('/').'/profile/'.$category.'/'.$province.'/'.$population.'/'.$title.'/'.$profile->id;
			echo $url;	
		}
