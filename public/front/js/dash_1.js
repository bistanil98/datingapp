...
function remove_double_slash(temp){
	...
	var get_profile_id = $('#get_profile_id').val();
	
			
		 /*	$.ajax({
			url: APP_URL+'ajax/visualizacione_chart' ,
			type: "get",
			data: {'profile_id' : get_profile_id},
...
var visualizaciones_chart = new ApexCharts(document.querySelector("#visualizaciones-chart"), visualizaciones);
var visualizaciones_url = remove_double_slash(APP_URL+'ajax/visualizacione_chart/'+get_profile_id);
visualizaciones_chart.render();
 $.getJSON(visualizaciones_url, function(response) {
     
  visualizaciones_chart.updateSeries([{
    name: 'times listed',
    data: response.data
  }])
}); 
...
var piesepuerto_chart = new ApexCharts(document.querySelector("#piesepuerto-chart"), piesepuerto);
var piesepuerto_url = remove_double_slash(APP_URL+'ajax/piesepuerto_chart/'+get_profile_id);
...
var top_anuncio_chart = new ApexCharts(document.querySelector("#top_anuncio-chart"), top_anuncio);

/*var top_anuncio_url = APP_URL+'ajax/top_anuncios_chart/'+get_profile_id;
top_anuncio_chart.render();
 $.getJSON(top_anuncio_url, function(response) {
     
  top_anuncio_chart.updateSeries([{
    name: 'Top',
    data: response.data
  }])
}); */
...
var subida_zone_chart = new ApexCharts(document.querySelector("#auto_subida-chart"), auto_subida);


var auto_subida_url = remove_double_slash(APP_URL+'ajax/subida_zone_chart/'+get_profile_id);
subida_zone_chart.render();
 $.getJSON(auto_subida_url, function(response) {
     
  subida_zone_chart.updateSeries([{
    name: 'Subida',
    data: response.data
  }])
}); 
...
"undefined" == typeof jQuery.migrateMute && (jQuery.migrateMute = !0),
    function(t) {
        ...
        s.migrateVersion = "3.1.0", n.console && n.console.log && (s && e("3.0.0") || n.console.log("JQMIGRATE: jQuery 3.0.0+ REQUIRED"), s.migrateWarnings && n.console.log("JQMIGRATE: Migrate plugin loaded multiple times"));
        var r = {};
...
(function() {
    ...
    function AjaxForm(form, callbackOnSuccess, callbackOnError, jsonResponse) {
        ...
        Ajax($form.attr('action'), $form.serialize(), callbackOnSuccess, callbackOnError, jsonResponse)
    }

    function RELOAD_WITH_ALIAS(alias) {
        $('<form method="post" action="' + location.href + '">' + '<input type="hidden" name="navegar_alias" value="' + alias + '">' + '</form>').appendTo(document.body).submit()
    }

    function setCookie(c_name, value, exdays) {
        var exdate = new Date();
        exdate.setDate(exdate.getDate() + exdays);
        var c_value = escape(value) + ((exdays == null) ? "" : "; expires=" + exdate.toUTCString()) + '; path=' + escape('/') + '; domain=slumi.com; SameSite=Strict';
    ...
    function SET_HOME_COOKIE(last) {
        ...
        try {
            var bStorage = /*!multiIdioma &&*/ ((typeof $ == 'function') ? $.support.localStorage : ((!!Storage) && (typeof localStorage.setItem == 'function')))
        } catch (e) {
            var bStorage = !1
        }
        if (last) {
    ...
    if (typeof $ == 'function') {
        ...
        try {
            $.support.localStorage = ((!!Storage) && (typeof localStorage.setItem == 'function'))
        } catch (e) {
            $.support.localStorage = !1
        }
        ...
        if (!$.support.positionFixed) $('html').addClass('nofixed');
    ...
    var timeout_resize_estadisticas = null;
    $.getScript(url, function() {
        google.charts.load('44', {
            'packages': ['corechart'],
            'language': ('es' === "mx" ? 'en' : 'es')
        });
        google.charts.setOnLoadCallback(cargarEstadisticas)
    });
    $(window).on('resize', redibujarEstadisticas);
    $(".estadisticas_ayuda").on("click", function(e) {
        $(this).toggleClass('hover');
        $($(this).closest(".bloque_estadisticas").find(".flip-container")).toggleClass('flip')
    });

    function redibujarEstadisticas() {
        clearTimeout(timeout_resize_estadisticas);
        timeout_resize_estadisticas = setTimeout(cargarEstadisticas, 150)
    }

    function dibujarEstadisticas(idChart, datos_chart) {
        var data = new google.visualization.DataTable(datos_chart.datos);
        var options = {
            tooltip:{trigger:'selection'},
            legend: (datos_chart.legend ? datos_chart.legend : 'none'),
            height: (datos_chart.height ? datos_chart.height : '295'),
            width: "100%",
            areaOpacity: 0.4,
            tooltip: {
                textStyle: {
            ...
            min: datos_chart.min,
            chartArea: {
                left: (datos_chart.chartAreaLeft ? datos_chart.chartAreaLeft : 62),
                right: 10,
                top: 27,
                width: '100%',
                height: 235,
            ...
            vAxis: {
                 baselineColor: '#ccc',
         gridlineColor: '#ccc',
                format: (datos_chart.vformat ? datos_chart.vformat : '#,###'),
                textStyle: {
                    fontSize: 11,
                    fontName: "Verdana",
                    color: '#fff'
                ...
                ticks: datos_chart.vticks
            },
            colors: (datos_chart.colors ? datos_chart.colors : ['#ff0000'])
        };
        if (datos_chart.bar) {
            options.bar = datos_chart.bar
        }
        if (datos_chart.isStacked) {
            options.isStacked = datos_chart.isStacked
        }
        if (datos_chart.tipoChart === 'SteppedAreaChart') {
            var chart = new google.visualization.SteppedAreaChart(document.getElementById(idChart))
        } else if (datos_chart.tipoChart === 'ColumnChart' || datos_chart.datos.rows.length === 1) {
    ...
    function cargarEstadisticas() {
        var estadisticas = {
             top: {
                idChart: "anuncio_estadisticas_top",
                selector: "#estadisticas_top_valores",
                datosDataTable: {
                    cols: [{
                        id: 'Dia',
                        label: "Día",
        ...
        for (var key in estadisticas) {
            var grafica = estadisticas[key];
			
           // var datos = $.parseJSON($('#estadisticas_top_valores').val());
			
            var datos_chart = {
                min: 3
            };
			var datos = {"Lunes, 2 de Marzo":"1","Martes, 3 de Marzo":"1","Mi\u00e9rcoles, 4 de Marzo":"1","Jueves, 5 de Marzo":"1","Viernes, 6 de Marzo":"1","S\u00e1bado, 7 de Marzo":"1","Domingo, 8 de Marzo":"0","Lunes, 9 de Marzo":"0","Martes, 10 de Marzo":"1","Mi\u00e9rcoles, 11 de Marzo":"1","Jueves, 12 de Marzo":"1","Viernes, 13 de Marzo":"1","S\u00e1bado, 14 de Marzo":"1","Domingo, 15 de Marzo":"1","Lunes, 16 de Marzo":"1","Martes, 17 de Marzo":"1","Mi\u00e9rcoles, 18 de Marzo":"1","Jueves, 19 de Marzo":"1","Viernes, 20 de Marzo":"1","S\u00e1bado, 21 de Marzo":"1","Domingo, 22 de Marzo":"1","Lunes, 23 de Marzo":"1","Martes, 24 de Marzo":"1","Mi\u00e9rcoles, 25 de Marzo":"1","Jueves, 26 de Marzo":"1","Viernes, 27 de Marzo":"1","S\u00e1bado, 28 de Marzo":"1","Domingo, 29 de Marzo":"1","Lunes, 30 de Marzo":"1","Martes, 31 de Marzo":"1","Mi\u00e9rcoles, 1 de Abril":"1"};
			//var myjson;
			/* $.getJSON('https://escortlisting.localjoy.website/ajax/top_anuncios_chart/159', function(datos) {
...
var hit_url =  remove_double_slash(APP_URL+'/ajax/top_anuncios_chart/'+get_profile_id) ;
var beer = $.ajax({
    url: hit_url,
    async: false,
    dataType: 'json'
  ...
  $.each(beer, function(key, item) {
       //console.log(item.y);
         grafica.datosDataTable.rows.push({
                            c: [{
                                v: item.name
                            }, {
                                v: parseInt(item.y),
                                f: (parseInt(item.y) ? 'Sí' : 'No')
                            }]
                        })
    });
		/*	 for (var key2 in datos) {
			 
                        grafica.datosDataTable.rows.push({
                            c: [{
                                v: key2
                            }, {
                                v: parseInt(datos[key2]),
                                f: (parseInt(datos[key2]) ? 'Sí' : 'No')
                            }]
                        })
                    } */
            ...
            datos_chart.datos = grafica.datosDataTable;
            for (var keyChart in grafica.datos_chart) {
                datos_chart[keyChart] = grafica.datos_chart[keyChart]
            }
            dibujarEstadisticas(grafica.idChart, datos_chart)
        }
    }
    $(document).ready(function() {
