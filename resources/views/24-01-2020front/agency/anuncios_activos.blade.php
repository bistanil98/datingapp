          ...
          <div class="left-image-text">
            ...
            <a href="{{url('agencia/delete/'.$free->id.'/anuncios-activos')}}" onclick="return confirm('Are you sure want to delete this item?')" class="icons-rt">delete</a>
				...
				<a href="{{url('agencia/status/'.$free->id.'/anuncios-activos')}}" onclick="return confirm('Are you sure want to change status this item?')" class="icons-rt deactive-red"> Disable </a> 
				 <?php } ?>
				<a href="{{url('agencia/subida-zone/'.$free->category.'/'.$free->id.'')}}" class="btn btn-danger"><i class="fas fa-arrow-circle-up"></i> Auto Subida</a>
            <a href="{{url('agencia/zona-top/'.$free->category.'/'.$free->id.'')}}" class="btn btn-danger"><i class="fas fa-arrow-circle-up"></i> Top Zona</a>
           </div>
          </div>
        </div>
      </div><!-- col-md-6 -->
        ...
        <div class="left-image-text-main green_01">
          ...
          <div class="left-image-text">
            ...
            <h5><i class="fa fa-mobile-phone"></i>  {{$subida->telephone}}</h5>
             <h5><span class="blak-block">Start Date {{date('d/m/Y',strtotime($check_top_subida_profile_lists->from_date))}}</span></h5> 
			 <h5><span class="blak-block">Expiry Date {{date('d/m/Y',strtotime($check_top_subida_profile_lists->to_date))}}</span></h5>           
           ...
           <a href="{{url('agencia/zona-top/'.$subida->category.'/'.$subida->id.'')}}" class="btn btn-danger"><i class="fas fa-arrow-circle-up"></i> Top Zona</a>
          ...
          <div class="left-image-text">
            ...
            <a href="{{url('agencia/delete/'.$top->id.'/anuncios-activos')}}" onclick="return confirm('Are you sure want to delete this item?')" class="icons-rt">delete</a>
				...
				<a href="{{url('agencia/status/'.$top->id.'/anuncios-activos')}}" onclick="return confirm('Are you sure want to change status this item?')" class="icons-rt deactive-red"> Disable </a> 
				 <?php } ?>
				  <a href="{{url('agencia/subida-zone/'.$top->category.'/'.$top->id.'')}}" class="btn btn-danger"><i class="fas fa-arrow-circle-up"></i> Auto Subida</a>
           </div>
          </div>
        </div>
      </div><!-- col-md-6 -->
