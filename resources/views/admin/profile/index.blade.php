					...
					<div class="row">
						<div class="col-md-12 col-lg-12">						
							 ...
							 <style> .card-header { display: block;padding: 1rem 0.5rem 0rem 2rem;}</style>
								...
								<form  action="{{url('/admin/profile/index/')}}/{{$category}}" enctype="multipart/form-data" method="get" >								
								
									<div class="row">
										
										<?php if($title=='Blocked Ads'){?>
										<?php }else if($title=='Ads with Selfies'){?>
										<?php }else if($title=='Free Ads'){?>
										...
										<?php if($title=='Blocked Ads' || $title=='Ads with Selfies'|| $title=='Free Ads'){?>
										<div class="col-md-2">
											<div class="form-group">
									...
									<div class="table-responsive">
										<table id="packages" class="table table-striped table-bordered text-nowrap w-100">
											<thead class="thead-light">												
												...
												<?php if($title=='Blocked Ads' || $title=='Ads with Selfies' || $title=='Free Ads'){?><th  class="text-center">Category</th><?php } ?>
											...
											@foreach($profile as $data)
												<tr>
													...
													<?php  if($title=='Blocked Ads' || $title=='Ads with Selfies' || $title=='Free Ads'){?><td>{{ ucfirst($data->category) }}</td><?php } ?>
													<td>{{ $data->email }} 
