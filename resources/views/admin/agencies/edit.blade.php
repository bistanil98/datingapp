...
<div class="container content-area relative">
	...
	<div class="row">
		<div class="col-lg-12">
			<form id="profile-ads" action="{{url('/admin/agencies/edit/')}}/{{$agencies->id}}" enctype="multipart/form-data" method="post" class="card">
				@csrf							
					...
					<div class="row">
						<div class="col-md-5"><!--left side -->
							<div class="form-group">
								...
								<?php /*<select name="agency_category" class="form-control">
									<?php foreach($categories as $data){?>
										<option @if($agencies->agency_category==$data->name) {{'selected'}} @endif value="{{$data->name}}">{{$data->name}}</option>
									<?php }?>
								</select>*/?>
								<?php foreach($categories as $data){?>
								<?php $checkmemberCategory = checkmemberCategory($agencies->id,$data->name);?>
							<div class="custom-control custom-checkbox custom-control-inline">
							<input <?php if($checkmemberCategory>0){ echo 'checked';}?> value="{{strtolower($data->name)}}" type="checkbox" class="custom-control-input" id="customCheck{{$data->id}}" name="agency_category[]">
					...
					<div class="row">
						...
						<div class="col-md-5"><!--left side -->
							...
							<div class="form-group">
								...
								<input value="{{$agencies->zona}}" type="text" class="form-control" name="zona" id="zona" placeholder="Enter zona">(Opcional)
					...
					<div class="row">          
						<div class="col-md-5">
							<div class="form-group">
								...
								<input value="{{$agencies->mobile}}" type="text" name="mobile" class="form-control">
					...
					<div class="row">          
						<div class="col-md-5"><!--left side -->
							<div class="form-group">
								...
								<input data-default-file="<?php if(!empty($agencies->upload_logo)){  echo URL::asset('public/uploads/profile_logo/'.$agencies->upload_logo); }?>" type="file" name="profile"  accept='image/*'  class="dropify">
