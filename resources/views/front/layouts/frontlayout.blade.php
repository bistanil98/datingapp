										...
										<li class="search-mainli">
											...
											<ul id="countries" class="sublist-ul country-list">
												...
												<?php foreach(countries() as $countriesData){?>
													...
													<li class="sublist-li country-flag <?php if(!empty($nacionalidad)){ if($nacionalidad[0]==rawurldecode(strtolower(str_replace(" ", ".", $countriesData->nationality)))){ echo 'active'; }	}?>"><a value='{{strtolower(str_replace(" ", ".", $countriesData->nationality) )}}' href="javascript:void(0);"><span class="countery-image"><img src="{{URL::asset('public/front/images/flags-medium/'.(strtolower($countriesData->alpha_2_code)).'.png')}}"></span> <span class="counter-name">{{$countriesData->nationality}}</span></a></li>
											...
											if(!empty($services)){
												...
												<?php foreach(services() as $servicesData){?>
													<li class="sublist-li firstcheckbox first_01">
														<label><input class="lesft-ab-check" type="checkbox" name="services[]" value="<?php echo strtolower(str_replace(" ", ".", $servicesData->name) );?>"<?php if(!empty($services)){ if( check_services_search($servicesData->name, $services)==1){ echo 'checked'; }	}?>> <span>{{$servicesData->name}}</span></label>
													</li> 
												<?php } ?>	
											</ul>
