@extends('front.layouts.agencia')
@section('content')
<section class="listing-panel-sec agency-dasboard-sec control-sec">
  <div class="container">
    <div class="row">
	@if(Session::has('message'))
	<p class="alert {{ Session::get('alert-class', 'alert-info') }}">{{ Session::get('message') }}</p>
	@endif			
      <div class="col-md-12">
        <div class="listing-div">
          <div class="row">
            <div class="col-md-12">
              <div class="listing-heading">
                <h2>PERFIL DE AGENCIA: {{$agency->email}}</h2>
              </div>
            </div>
             	
             <div class="col-md-4 col-lg-3 col-sm-6">
              <a href="{{url('agencia/create-add')}}">
              <div class="listing-icon-text icon-1">
                <!-- <i class="fas fa-meteor"></i> --><span class="icon-font control-text">Crear Anuncios</span>
              </div>
               </a>
            </div>
             <!-- <div class="col-md-4 col-lg-3 col-sm-6">
              <a href="#">
              <div class="listing-icon-text icon-3">
                <i class="fas fa-bullhorn"></i> <span style="font-size:20px">ANUNCIOS CREADOS</span>
              </div>
              </a>
            </div> -->
             <div class="col-md-4 col-lg-3 col-sm-6">
             <a href="{{url('agencia/anuncios-activos')}}">
              <div class="listing-icon-text icon-2">
                <!-- <i class="fas fa-sync"></i> --> <span class="icon-font control-text">Mis Anuncios</span>
              </div>
              </a>
            </div>
            <div class="col-md-4 col-lg-3 col-sm-6">
               <a href="{{url('agencia/zona-top')}}">
              <div class="listing-icon-text icon-3">
                <!-- <i class="fas fa-list"></i> --> <span class="icon-font control-text">Contratar<br/> Top Anuncios</span>
              </div>
                </a>
            </div>

			<div class="col-md-4 col-lg-3 col-sm-6">
              <a href="{{url('agencia/subida-zone')}}">
              <div class="listing-icon-text icon-4">
                <!-- <i class="fas fa-list"></i> --> <span class="icon-font control-text">Contratar<br/> Auto Subidas</span>
              </div>
                </a>
            </div>

			<div class="col-md-4 col-lg-3 col-sm-6">
                <a href="{{url('agencia/consumo')}}">
              <div class="listing-icon-text icon-5">
                <!-- <i class="fas fa-list"></i> --> <span class="icon-font control-text">Consumo</span>
              </div>
                </a>
            </div>

			<div class="col-md-4 col-lg-3 col-sm-6">
                <a href="{{url('agencia/facturacion')}}">
              <div class="listing-icon-text icon-6">
                <!-- <i class="fas fa-list"></i> --> <span class="icon-font control-text">Facturaci&oacute;n</span>
              </div>
                </a>
            </div>

			<div class="col-md-4 col-lg-3 col-sm-6">
                <a href="{{url('agencia/datos-de-agencia')}}">
              <div class="listing-icon-text icon-7">
                <!-- <i class="fas fa-list"></i> --> <span class="icon-font control-text" style="text-transform:none !important">Datos de Agencia</span>
              </div>
                </a>
            </div>     
          
		  </div>
        </div>
      </div>
    </div>

    <div class="row">
      <div class="col-md-12">
        <div class="row">
          <div class="col-md-3 m-auto d-block">
            <div class="listing-center-text">
              <p>Truvalia Global Classifieds OOD
				Iskar 4, 1000 Sofia, Bulgaria</p>
            </div>
          </div>
        </div>
      </div>


    </div>

  </div>
</section>


@endsection