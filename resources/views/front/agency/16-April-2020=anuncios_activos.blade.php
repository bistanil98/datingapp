      ...
      <div class="col-md-8 col-lg-6">
        <div class="left-image-text-main">
          ...
          <div class="left-image-text">
            ...
            <h5>Fecha de creación {{date('d M Y',strtotime($free->created_at))}}</h5>            
	   ...
	   <div class="col-md-8 col-lg-6">
        <div class="left-image-text-main green_01">
          ...
          <div class="left-image-text">
			...
			<h5><span class="blak-block">Fetcha de inicio :  {{date('d-m-Y',strtotime($check_top_subida_profile_lists->from_date))}}</span></h5>             
            <h5>Horario de subidas : <span class="labl-txt">{{date('H:i',strtotime($check_top_subida_profile_lists->from_date_time))}} - {{date('H:i',strtotime($check_top_subida_profile_lists->to_date_time))}}</span></h5>
            <h5>Subidas diarias :  <span class="labl-txt">{{$check_top_subida_profile_lists->seen_days}} dias</span></h5>                        
			 ...
			 <?php }else{
				...
				?>
				 ...
				 <h5>Dias restantes :  <span style="color:#28a745" class="blak-block"><?php echo $totalleftdays;?> Days left</span></h5>    
			 <?php } }?>
            <div class="icons-right">
				<?php if($check_top_subida_profile_lists->from_date>date('Y-m-d')){ ?>
					 <?php if($subida->user_status_subida==1){?>
					<a href="javascript:void(0)"  class="icons-rt deactive-red disabled-btn"> Desactivar </a>
					<?php }else{?>
					<a href="javascript:void(0)"  class="icons-rt active-green disabled-btn"> Reactivar </a> 
					<?php }?>
			 <?php }else{ ?>
					 <?php if($subida->user_status_subida==1){?>
						<a href="{{url('agencia/status/'.$subida->id.'/anuncios-activos/'.$top_subida_profile_lists_id.'/subida')}}" onclick="return confirm('Are you sure want to change status this item?')" class="icons-rt deactive-red"> Desactivar </a>
						 <?php }else{?>
        ...
        <div class="left-image-text-main blue_01">
          ...
          <div class="left-image-text">
            ...
            <div class="icons-right">			
			 ...
			 <?php }else{ ?>
				<?php if($top->user_status==1){?>
				<a href="{{url('agencia/status/'.$top->id.'/anuncios-activos/'.$top_subida_profile_lists_id.'/top')}}" onclick="return confirm('Are you sure want to change status this item?')" class="icons-rt deactive-red"> Desactivar </a>
				<?php }else{?>
