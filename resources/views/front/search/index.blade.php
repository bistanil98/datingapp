...
<div id="searchResult">
			...
			<div class="container-fluid">
				<div class="row">
						...
						<div class="video-header-text">
							...
							<nav aria-label="breadcrumb">
								<ol class="breadcrumb">
									...
									<?php $population = population($location[0]);
										if(!empty($population)){
										if(strtolower($population)==$location[0]){ ?>										
										<?php }else{ ?>
										<li class="breadcrumb-item active" aria-current="page">{{ucwords($population)}}</li>
											<?php } ?>
										<?php } ?>
										<li class="breadcrumb-item active" aria-current="page">{{ucwords($location[0])}}</li>
										<?php }?>
								</ol>
							</nav>
						</div>
					...
					<div class="col-md-5  order-column-icon">
						<div class="icon-div">
							<a href="#" class="alta-button favrote" data-toggle="modal" data-target="#counter"><i class="fas fa-heart" ></i><span class="counterone" id="total_favoritas">{{favoritos_count()}}</span> Favoritos</a>
	...
	<?php }?>
		...
		<input value="<?php if(!empty($top_zona_ids)){ echo implode(",", $top_zona_ids);}?><?php if(!empty($subida_ids)){ echo ','.implode(",", $subida_ids);}?><?php if(!empty($get_subida_going_address_ids)){ echo ','.implode(",", $get_subida_going_address_ids);}?>" id="ads_list_ids" type="hidden" />
